"""
Strategy 022b — SuperTrend(ATR=2.0) on EURUSD 1h.
Entry on green SuperTrend + 5-day aggregate confirmation.

Entry rules (at the close of bar t on 1h):
  LONG when SuperTrend is green
         AND the 5-day aggregate is green
             (daily close[t-1] > daily open[t-5], so the net move
              over the last 5 completed days is up; individual days
              within the window may be red)
...
def build_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    daily = out.groupby(date).agg(d_open=("Open", "first"),
                                  d_close=("Close", "last"))
    # 5-day aggregate: close of yesterday vs open of 5 days ago.
    daily["agg5_green"] = (daily["d_close"].shift(1) > daily["d_open"].shift(5)).astype("Int8")

    out["agg5_green"] = pd.Series(date, index=out.index).map(daily["agg5_green"]).astype("Int8")

    out["enter_long"] = ((out["st_green"] == 1) &
                         (out["today_green"] == 1) &
                         (out["agg5_green"]  == 1)).astype(bool)
    return out
...
if __name__ == "__main__":
    ...
    sig = sig.dropna(subset=["st", "agg5_green"]).copy()
