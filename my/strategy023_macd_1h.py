"""
Strategy 023b — MACD(12,26,9) crossover on EURUSD 1h, US session only.

Entry rules (at the close of bar t on 1h, only if 13:00 <= UTC hour < 21:00):
...
MAX_HOLD      = 200  # ~25 US-session days on 1h
...
def report(m_all: dict) -> None:
    hdr = (f"{'side':>6} {'n':>5} {'pips':>9} {'sharpe':>7} {'win%':>6} "
           ...
           f"{'tp%':>5} {'sl%':>5} {'tim%':>5}")
    print(f"\n[1h] MACD({MACD_FAST},{MACD_SLOW},{MACD_SIGNAL}) crossover, US session results")
    print(hdr)
    print("-" * len(hdr))
