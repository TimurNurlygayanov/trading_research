"""
Strategy 018 — Regime-conditioned RF buy classifier on EURUSD 1h.
1h version of strategy 015 (1:3 R:R, TP = 60 pips, SL = 20 pips).

Pipeline:
  1. Load 2025 1h data (train) and 2026 1h data (test) from MT5.
  ...
  4. Per-bar binary label: 1 if High >= entry + TP before Low <= entry - SL
     within 200 forward bars (SL priority on same-bar tie).
        TP = 60 pips, SL = 20 pips, 1-pip spread deducted at exit.
...
TP = +60 pips, SL = -20 pips, max hold = 200 bars (~8.3 days on 1h).
Spread (1 pip) is deducted at exit time.
"""
import sys
import time
...
def get_data(ticker: str, start: str, end: str) -> pd.DataFrame:
    ...
    frames, pos = [], 0
    while True:
        chunk = mt5.copy_rates_from_pos(ticker, _TF_H1, pos, CHUNK)
