"""
Strategy 024 — Strong-body impulse candle on EURUSD 5m, long only.

Entry rules (at the close of bar t on 5m):
  LONG when current bar is green AND body > 1.2 * ATR(14)
         AND the previous completed daily candle is green
         AND the current (forming) daily candle is green so far

Exit: SL = entry-bar Low, TP = entry + 3 * (entry - SL).
"""
import sys
...
def build_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    strong = body > BODY_ATR * out["atr"].values
    out["green_strong"] = pd.Series((close > open_) & strong, index=out.index).astype("Int8")
    ...
    out["enter_long"] = ((out["green_strong"] == 1) &
                         (out["today_green"]  == 1) &
...
def report(m_all: dict) -> None:
    hdr = (f"{'side':>6} {'n':>5} {'pips':>9} {'sharpe':>7} {'win%':>6} "
           ...
           f"{'tp%':>5} {'sl%':>5} {'tim%':>5}")
    print(f"\n[5m] Strong-body impulse (body > {BODY_ATR}*ATR), long only")
    print(hdr)
    print("-" * len(hdr))
    _print_row("long", m_all)
...
if __name__ == "__main__":
    ...
    sig = sig.dropna(subset=["atr", "prev_green"]).copy()
