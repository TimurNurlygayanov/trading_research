"""
Strategy 023a — MACD(12,26,9) crossover on EURUSD 5m, US session only.

Entry rules (at the close of bar t on 5m, only if 13:00 <= UTC hour < 21:00):
...
Exit: SL = entry - 1.5*ATR(14), TP = entry + 4.5*ATR(14)  (1:3 R:R).
"""
import sys
...
SL_ATR        = 1.5
TP_ATR        = 4.5
US_HOUR_START = 13  # UTC
US_HOUR_END   = 21  # UTC (exclusive)
MAX_HOLD      = 288  # ~1 trading day on 5m

_TF = {"5m": mt5.TIMEFRAME_M5}
...
def _macd(close: np.ndarray, fast: int, slow: int, signal: int):
    ...
    # Signal EMA needs to handle NaNs at the start of macd_line.
    valid_from = slow - 1
    sig = np.full_like(macd_line, np.nan)
    if len(macd_line) - valid_from >= signal:
        sig_seed = macd_line[valid_from:valid_from + signal].mean()
        sig[valid_from + signal - 1] = sig_seed
        a = 2.0 / (signal + 1.0)
        for i in range(valid_from + signal, len(macd_line)):
            sig[i] = a * macd_line[i] + (1 - a) * sig[i - 1]
...
def build_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    macd_line, sig_line = _macd(close, MACD_FAST, MACD_SLOW, MACD_SIGNAL)
    out["macd"]   = macd_line
    out["signal"] = sig_line

    macd_s = pd.Series(macd_line, index=out.index)
    sig_s  = pd.Series(sig_line,  index=out.index)
    bullish_cross = (macd_s > sig_s) & (macd_s.shift(1) <= sig_s.shift(1))
    out["macd_cross"] = bullish_cross.astype("Int8")
    ...
    out["us_session"] = ((hour >= US_HOUR_START) & (hour < US_HOUR_END)).astype("Int8")

    out["enter_long"] = ((out["macd_cross"] == 1) &
                         (out["today_green"] == 1) &
                         (out["prev_green"]  == 1) &
                         (out["us_session"]  == 1)).astype(bool)
    return out
...
def report(m_all: dict) -> None:
    hdr = (f"{'side':>6} {'n':>5} {'pips':>9} {'sharpe':>7} {'win%':>6} "
           ...
           f"{'tp%':>5} {'sl%':>5} {'tim%':>5}")
    print(f"\n[5m] MACD({MACD_FAST},{MACD_SLOW},{MACD_SIGNAL}) crossover, US session results")
    print(hdr)
    print("-" * len(hdr))
